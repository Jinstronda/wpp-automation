...
async function clickAnyWithinRoot(root: Locator, selectors: string[]): Promise<boolean> {
  for (const selector of selectors) {
...
async function typeIntoWithinRoot(root: Locator, page: Page, selectors: string[], value: string): Promise<boolean> {
  for (const selector of selectors) {
    const el: Locator = root.locator(selector);
    if (await el.count()) {
      try {
...
async function createContactAndMessage(
  ...
  if (!newContactClicked) {
    ...
    await page.evaluate(() => {
      const el = Array.from(document.querySelectorAll('*')).find(e => e.textContent?.trim() === 'New contact') as HTMLElement | undefined;
      if (el) el.click();
    });
  }
